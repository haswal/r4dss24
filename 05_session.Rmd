...
1. Calculate the minimum Volume for each cloudbuddy and phase. Next, calculate the mean of all the minimum values you just generated (the output should contain one value only). Does it make a difference if you remove NAs in the beginning of the pipeline using `filter()`, if you use `na.rm = TRUE` when calculating the minimum values, or if you use `na.rm = TRUE` when calculating the mean? Pay attention to messages produced (warnings and such) when trying the three different options. Explain (preferably with code examples) any differences you see in the output. 
